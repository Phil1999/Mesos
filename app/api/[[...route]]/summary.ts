        ...
        async (c) => {
            ...
            async function fetchFinancialData(
                ...
                return await db
                    .select({
                        income: sql`
                            SUM(CASE WHEN ${transactions.amount} >= 0 THEN ${transactions.amount}
                            ELSE 0 END)
                        `.mapWith(Number),
            ...
            .where(
                and(
                    ...
                    accountId? eq(transactions.accountId, accountId) : undefined,
                    lt(transactions.amount, 0), // Only count positive categories spent
                    gte(transactions.date, startDate),
                    lte(transactions.date, endDate),
                )
            )
            ...
            // Display top 3 categories and all else will be in "other"
            ...
            const activeDays = await db
                .select({
                    ...
                    expenses: sql`
                        SUM(CASE WHEN ${transactions.amount} < 0 THEN ABS(${transactions.amount})
                        ELSE 0 END)`.mapWith(Number),
